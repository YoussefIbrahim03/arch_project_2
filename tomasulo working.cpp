...
int get_line(string name)
{
    ...
    {
        cout << branch[i].first << endl<<endl <<endl;
...
void CALL(string name, int bprogcount, int& r1,int& programcount)
{
    ...
    cout << "counter: " << branchnum << endl;
            programcount=branchnum;
}
...
void write()
{
    for(int i=0;i<15;i++)
    {
        ...
        else
        {
            ...
            {
                cout << rs[i].writeflag << (instructions[rs[i].instruction].exec_end!=clock_cycle) <<(rs[i].operation=="CALL") << endl;
